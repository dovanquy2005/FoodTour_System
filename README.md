...
Hệ thống được xây dựng trên nền tảng **.NET 9**, chia làm 2 thành phần chính:**1. Web Admin & API (Backend):*** **Lưu trữ Cloud:** PostgreSQL lưu trữ trên điện toán đám mây **Supabase**.* **ORM:** Entity Framework Core.* **Giao diện quản trị:** Blazor Server + MudBlazor.**2. Mobile App (Frontend):*** **📍 Bản đồ:** `Microsoft.Maui.Controls.Maps` (Tích hợp Google Maps chuyên sâu).* **🏗 Mô hình:** `CommunityToolkit.Mvvm` (Giúp code sạch, dễ bảo trì theo chuẩn MVVM).* **💾 Lưu trữ Offline:** `sqlite-net-pcl` & `SQLitePCLRaw.bundle_green` (Quản lý database SQLite).* **⚡ Tính năng lõi:** Tích hợp sẵn GPS (Geolocator), Giọng nói (Text-to-Speech), và Rung (Vibration).
...
Sử dụng các lệnh .NET CLI sau trong thư mục `FoodTour_WebAdmin.Api` để quản lý database:```bash
...
# Di chuyển tới thư mục APIcd d:\FoodTour_System\FoodTour_WebAdmin.Api# Cập nhật cấu trúc lên Database Cloud (Supabase)

dotnet ef database update# Chạy Server

dotnet run# Dừng server

taskkill /IM "FoodTour_WebAdmin.Api.exe" /F

Cổng mặc định: http://localhost:5154 - Bạn có thể truy cập http://localhost:5154/api/shops để kiểm tra dữ liệu JSON. Hoặc kiểm tra trên môi trường Production tại link Render.
